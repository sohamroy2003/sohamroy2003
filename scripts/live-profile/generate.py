...
SOURCE = ROOT / "assets/source/soham.png"
ASSETS = ROOT / "assets"
...
ROWS = [
    ...
    ("Focus", "AI / ML · Analytics · GenAI"),
    ("Education", "B.Tech CSE · AI & ML"),
    ("Status", "Building + Learning + Shipping"),
    ("Core.Lang", "Python · SQL"),
    ("Core.Data", "Pandas · NumPy · Power BI"),
    ("Core.ML", "Scikit-learn · TensorFlow · PyTorch"),
    ("Core.GenAI", "LLMs · RAG · AI Agents"),
    ("Core.Database", "PostgreSQL · MySQL · MongoDB"),
    ("Core.Cloud", "AWS · Docker · Git"),
    ("Grid.LinkedIn", "/in/sohamroy21"),
    ("Grid.GitHub", "sohamroy2003"),
    ("Grid.Kaggle", "sohamroy2003"),
    ("Grid.Mail", "business.ash20@gmail.com"),
]
...
def make_logos() -> dict[str, Image.Image]:
    ...
    size=400; logos={}
    data=Image.new("RGBA",(size,size),(0,0,0,0)); d=ImageDraw.Draw(data)
    d.ellipse((78,65,322,155),fill="black"); d.rectangle((78,110,322,290),fill="black"); d.ellipse((78,245,322,335),fill="black")
    for y in (150,215,280):
        d.ellipse((98,y-24,302,y+24),fill=(0,0,0,0)); d.rectangle((98,y-24,302,y),fill=(0,0,0,0))
    logos["rust"]=data
    ai=Image.new("RGBA",(size,size),(0,0,0,0)); d=ImageDraw.Draw(ai)
    layers=[[(80,120),(80,200),(80,280)],[(200,90),(200,160),(200,240),(200,310)],[(320,140),(320,260)]]
    for a in layers[0]:
        for b in layers[1]: d.line([a,b],fill="black",width=12)
    for a in layers[1]:
        for b in layers[2]: d.line([a,b],fill="black",width=12)
    for layer in layers:
        for x,y in layer: d.ellipse((x-24,y-24,x+24,y+24),fill="black")
    logos["code"]=ai
    ml=Image.new("RGBA",(size,size),(0,0,0,0)); d=ImageDraw.Draw(ml)
    d.rectangle((70,250,125,325),fill="black"); d.rectangle((150,205,205,325),fill="black"); d.rectangle((230,150,285,325),fill="black"); d.rectangle((310,85,365,325),fill="black")
    d.line([(70,220),(175,170),(255,115),(345,60)],fill="black",width=18); d.polygon([(345,60),(310,64),(338,92)],fill="black")
    logos["stellar"]=ml
    for name,image in logos.items(): image.save(LOGOS/f"{name}.png",optimize=True)
    return logos
...
def portrait_points(theme: str, rng: np.random.Generator) -> np.ndarray:
    ...
    sw, sh = source.size
    crop = source.crop((int(sw*0.06), int(sh*0.06), int(sw*0.94), int(sh*0.73))).resize((300, 340), Image.Resampling.LANCZOS)
...
def render_svg(
    ...
    parts: list[str] = [
        ...
        "<title id=\"title\">Soham's live AI and data profile</title>",
    ...
    parts.extend(
        [
            ...
            'font-weight="700">@sohamroy2003</text>',
        ]
    )
...
def main() -> None:
    ...
    for index, theme in enumerate(THEMES):
        ...
        output = ASSETS / theme / "live-profile.svg"
